...
Read `.tmp/quickening-1-elder-quiz.md`. Walk the listed references next — for large code files, scope first (greps, function list, scope hints from the quiz) and read only the relevant parts. Then investigate the codebase to answer each question. Don't guess.
...
Save answers to `.tmp/quickening-2-scion-attempt.md`. After answering, append any open questions that came up during investigation.
